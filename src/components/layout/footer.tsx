...
export function Footer() {
  return (
    <footer className="bg-black text-carbon-100">
      <div className="mx-auto max-w-7xl px-5 py-16 sm:px-6 lg:px-8 lg:py-20">
          ...
          {footerNav.map((col) => (
            <nav key={col.title} aria-label={col.title}>
              ...
              <ul className="mt-4 space-y-2.5">
                {col.links.map((link) => (
                  <li key={link.label}>
                    <Link href={link.href} className="text-sm text-carbon-100/75 transition-colors hover:text-carbon-50">
                      {link.label}
                    </Link>
                  </li>
                ))}
        ...
        <div className="mt-14 flex flex-col gap-4 border-t border-white/10 pt-7 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-wrap items-center gap-x-5 gap-y-2">
            {site.socials.map((s) => (
              <a key={s.label} href={s.href} target="_blank" rel="noreferrer" className="text-sm text-carbon-100/70 transition-colors hover:text-carbon-50">
                {s.label}
              </a>
            ))}
          </div>
          <div className="flex flex-wrap items-center gap-x-5 gap-y-2">
            ...
            <p className="text-xs text-carbon-100/45">© {new Date().getFullYear()} Framies</p>
          </div>
        </div>
      </div>
    </footer>
